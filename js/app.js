...
const me = { nick: '', hash: '' };
let rooms = [], dms = [];
let active = LS.get('active', null);            // {kind:'room'|'dm', id, name}
let shown = new Map(), lastId = 0, pollTimer = null, sweeping = 0;
let draft = null;                                // подготовленное фото {blob,path,url}
...
const left = (ts) => {
  ...
  return h <= 0 ? 'istёк' : `осталось ${h > 47 ? '≈48' : h} ч`;
};
...
function bubble(m) {
  ...
  if (m.media) {
    ...
    img.decoding = 'async';
    img.onerror = () => img.remove();          // не оставляем пустую рамку, если байты не дошли
...
async function pull() {
  ...
  } catch (e) { /* сеть мигнула — попробуем на следующем тике */ }
...
/* сжимаем фото на клиенте: с телефона приходят тяжёлые оригиналы и HEIC */
async function fileToBlob(file) {
  const bmp = await createImageBitmap(file, { imageOrientation: 'from-image' }).catch(() => null);
  if (!bmp) return file.type.startsWith('image/') ? file : null;   // HEIC и прочее, что браузер не декодирует
...
async function attach(file) {
  ...
  // вложение едет прямо в строке сообщения, поэтому жмём до потолка CHECK (400k символов base64)
...
function grow() {
  ...
  const need = Math.max(t.scrollHeight, t.offsetHeight);   // placeholder короче одной строки — не режем
...
async function enter() {
  // сервер обязан подтвердить пару ник+код, иначе страница притворяется залогиненной
  const v = await api.rpc('chat_auth', { p_nick: me.nick, p_code_hash: me.hash });
  const r = Array.isArray(v) ? v[0] : v;
  if (!r?.ok) {
    sessionStorage.removeItem('eschat.me');
    me.nick = me.hash = '';
    showLogin(r?.error === 'nick_taken' ? 'Такой ник уже занят и закреплён другим кодом.'
                                        : 'Код не подошёл. Войди по своему коду.');
